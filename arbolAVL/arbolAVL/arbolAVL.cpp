...
protected:
    ...
    int esAVL_rec(T min, T max, Link raiz) {
        if (raiz->elem <= min || (max != -1 && raiz->elem >= max)) return -1;

        int altIzq = 0;
        if (raiz->left != nullptr) {
            altIzq = esAVL_rec(min, raiz->elem, raiz->left);
            if (altIzq == -1) return -1;
        }

        int altDer = 0;
        if (raiz->right != nullptr) {
            altDer = esAVL_rec(raiz->elem, max, raiz->right);
            if (altDer == -1) return -1;
        }

        if (abs(altIzq - altDer) > 1) return -1;

        if (altIzq == 0 && altDer == 0) return 1;
        else return std::max(altIzq, altDer) + 1;
    }
...
bintree<int> construyeArbol(int raiz) {
    int rI;
    cin >> rI;
    bintree<int> hijoIzq;
    if (rI != -1) hijoIzq = construyeArbol(rI);

    int rD;
    cin >> rD;
    bintree<int> hijoDer;
    if (rD != -1) hijoDer = construyeArbol(rD);

    return bintree<int> (hijoIzq, raiz, hijoDer);
}
...
void resuelveCaso() {
    ...
    cin >> raiz;
    if (raiz != -1) {
        bintree<int> arbol = construyeArbol(raiz);

        if (arbol.esAVL() == -1) cout << "NO\n";
        else cout << "SI\n";
    }
    else cout << "SI\n";
}
